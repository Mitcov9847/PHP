...
```
**Пояснение:** В этом цикле переменные `$a` и `$b` изменяются на каждом шаге, увеличиваясь соответственно на 10 и 5. Выводится их значение после каждого шага.
...
?>
```
**Пояснение:** `$i` объявляется перед циклом и увеличивается внутри тела цикла.
